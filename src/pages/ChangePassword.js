...
//   const navigate = useNavigate();
  const { user } = useAuth(); // Assuming `user` provides `_id`
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (json.status) {
        ...
        setError("");
        // navigate("/dashboard"); // Navigate to a relevant page
      } else {
  ...
  return (
    <>
      <div className="mx-auto grid place-items-center h-[80dvh] ">
       
        <div>
          <div className="flex justify-center items-center gap-2 mb-4">
            <img src={changePasswordIcon} className="w-16 block" alt="" />
            <h1 className="text-xl md:text-2xl font-medium">Change Password</h1>
          </div>
          <div className="shadow-md bg-slate-50/10 backdrop-blur-md p-5 rounded-lg border-t-4 border-secondary w-[400px] sm:w-[500px]">
            <h1 className="text-xl font-bold my-4">Update your password</h1>
            <form onSubmit={handleSubmit} className="flex flex-col gap-3">
              ...
              <button className="bg-secondary text-white py-2 font-bold rounded cursor-pointer">
                <p>{loading ? "Updating..." : "Change Password"}</p>
              </button>
              {error && (
                <div>
